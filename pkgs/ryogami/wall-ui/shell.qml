...
ShellRoot {
    ...
    function _applyPendingBrowse() {   // PARCHE
        ...
        if (!item) return
        item.browseSource = root._pendingBrowse
        item.browseOpen = true
        root._pendingBrowse = ""
    }
    ...
    Connections {
        ...
        // PARCHE: abrir directo en una fuente remota (wallhaven)
        function onWallpaperBrowse(source) {
            root._pendingBrowse = source || "wallhaven"
            root._setShowing(true)
