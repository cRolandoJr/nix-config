...
Scope {
  ...
  WallpaperSelectorService {
    ...
    onModelUpdated: {
      ...
      if (service.filteredModel.count > 0) {
        // Default: quedarse donde esta, no saltar al principio. Este handler
        // tambien corre por refrescos que el usuario no pidio (el daemon manda
        // datos nuevos al cachear thumbs o al detectar cambios en la carpeta),
        // y ahi _preCommitIndex vale -1: con `var idx = 0` el carrusel se iba
        // solo al primer wallpaper en medio de la navegacion.
        var idx = (wallpaperSelector.showing && sliceListView.currentIndex > 0)
          ? Math.min(sliceListView.currentIndex, service.filteredModel.count - 1)
          : 0
