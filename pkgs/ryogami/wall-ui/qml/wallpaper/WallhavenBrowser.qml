...
Item {
  ...
  Timer {
    id: gridFocusTimer
    interval: 50
    onTriggered: resultsGrid.forceActiveFocus()
  }
  ...
  Shortcut {
    ...
    onActivated: resultsGrid._moveSelection(-1)
  }
  Shortcut {
    sequences: ["Right"]
    enabled: browser.browserVisible && !browser._previewOpen
  ...
  Shortcut {
    sequences: ["Up"]
    enabled: browser.browserVisible && !browser._previewOpen
    onActivated: resultsGrid._moveSelection(-Math.max(1, Config.wallhavenColumns))
  }
  Shortcut {
    sequences: ["Down"]
    enabled: browser.browserVisible && !browser._previewOpen
  ...
  Shortcut {
    sequences: ["Shift+Left", "PgUp"]
    enabled: browser.browserVisible && !browser._previewOpen
    onActivated: browser.pagePrev()
  }
  Shortcut {
    sequences: ["Shift+Right", "PgDown"]
    enabled: browser.browserVisible && !browser._previewOpen
    onActivated: browser.pageNext()
  }
  ...
  GridView {
    ...
    // Navegacion explicita: mover currentIndex a mano en vez de confiar en la
    // del GridView, que con interactive:false no responde. currentIndex es lo
    // que dibuja el borde del delegate, asi que la seleccion se ve sola.
    function _moveSelection(delta) {
      var n = resultsModel.count
      if (n <= 0) return
  ...
  Text {
    ...
    text: (browser.whService && browser.whService.query !== "")
      ? "Sin resultados para \"" + browser.whService.query + "\" con estos filtros"
      : "Search wallhaven.cc for wallpapers, or browse the top list"
  ...
  onBrowserVisibleChanged: {
    // Sin foco el grid no recibe ninguna tecla: solo lo tomaba al hacer click.
    if (browserVisible) gridFocusTimer.restart()
