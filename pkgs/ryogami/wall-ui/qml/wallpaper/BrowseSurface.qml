...
Item {
  ...
  readonly property var _sources: {
    ...
    // s.push({ key: "moewalls", label: "MOEWALLS" })    // falta moewalls-search
    // s.push({ key: "motionbgs", label: "MOTIONBGS" })  // falta motionbgs-search
    // s.push({ key: "ryostore", label: "RYOSTORE" })    // falta extras-search
    // s.push({ key: "repos", label: "REPOS" })          // falta library-list
    return s
  }
