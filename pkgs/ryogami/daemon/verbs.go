...
func (d *daemon) wallpaperVerb(line string) string {
	...
	case "ui":
		...
		return "ok"
	case "browse":
		// PARCHE: abre el picker directamente en una fuente remota
		// (wallhaven por defecto). El QML escucha ryogami.wall.browse.
		src := strings.TrimSpace(strings.TrimPrefix(rest, "browse"))
		if src == "" {
			src = "wallhaven"
		}
		started := d.ui.ensure()
		payload := map[string]interface{}{"source": src}
		d.broadcast("ryogami.wall.browse", payload)
		if !started {
			// Arranque en frio: el QML todavia no estaba suscrito.
			go func() {
